...
void Scene::render(IEngineCore* engineCore)
{
	...
	// keep the old mouse position and button state (could be a method)
	m_oldMouseX = m_mouseX;
	m_oldMouseY = m_mouseY;
	m_oldMouseButtons = m_mouseButtons;
	...
	// ** I've modified this to use bit 1 so it doesn't interfere with IMGUI **
	// do we have a new button press (has bit 1 changed from 0)
	// i.e is bit 1 not 0 and old bit 1 == is 0
	// the (!= 0) is there for clarity!
...
bool Scene::loadLevelJSON(std::string levelJSONFile)
{
	...
	//v_GameObjects.resize(numberOfCubes); - not with pointers
	...
	for (int i = 0; i < (int)gameObjects.size(); i++)
	{
		...
		if (scaleNode.type() != Json::nullValue)
		{
			...
			// better to leave objects unscaled unless shader re-normalises normals else lighting is broken
		}

		// todo - fix this to be data dependent (it's hardwired at the minute)
		if (i == m_playerIndex)
		{
